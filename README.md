...
| `SGCOMP` | Singapore + immediate Strait | MSS Singapore (Changi, 50 km / 5-min) | 0.0045° (~0.5km) | <1 MB |
...
Group aliases: `CONUS` (continental US only), `US` (all US regions), `CANADA` (Canada), `CENTRAL_AMERICA` (El Salvador + W. Honduras + S. Guatemala + offshore Pacific), `EUROPE` (OPERA pan-European composite), `SOUTHEAST_ASIA` (MSS Singapore 50 km + MET Malaysia peninsular + east composites — Singapore, Peninsular Malaysia, Borneo, Brunei, N. Sumatra), `TAIWAN` (CWA QPESUMS composite covering Taiwan + offshore Pacific), `ALL` (everything).
...
- **SE Asia (Singapore):** MSS (Meteorological Service Singapore) 50 km high-resolution rain-area radar via anonymous HTTPS at `weather.gov.sg` — 217×120 RGBA PNG at ~0.5 km / 5-min cadence centered on MSS Changi, decoded via discrete palette → dBZ lookup. Sharper than MET Malaysia over Singapore proper. Singapore Open Data Licence v1.0 (data.gov.sg / MSS / NEA) — attribution required.
- **SE Asia (Malaysia, Borneo, Brunei):** MET Malaysia (Jabatan Meteorologi Malaysia) 12-radar national composite via anonymous HTTPS at `api.met.gov.my` — 1352×570 animated GIF carrying 6 frames at 10-min cadence (~60 min of backfill per fetch), decoded via 18-stop palette → dBZ table. Split into two regions (`MYPENINSULAR` and `MYEAST`) covering Peninsular Malaysia + N. Sumatra and East Malaysia (Borneo) + Brunei respectively. CC-BY-4.0 — attribution required.
...
- **[MSS Singapore](https://www.weather.gov.sg/)** — Meteorological Service Singapore (under NEA), 50 km high-resolution rain-area product (`dpsri_70km`) via anonymous HTTPS at `weather.gov.sg`. Licensed under the [Singapore Open Data Licence v1.0](https://beta.data.gov.sg/open-data-license) (Data © Data.gov.sg © Meteorological Service Singapore © National Environment Agency).
...
- **Limited radar coverage outside US / Canada / Europe / Central America / Taiwan / SE Asia** — real radar composites cover the US (CONUS, Alaska, Hawaii, Puerto Rico, Guam), Canada, El Salvador and its neighbours, Europe (via OPERA), Taiwan (CWA QPESUMS), Singapore (MSS), and Malaysia + Borneo + Brunei + N. Sumatra (MET Malaysia). Everywhere else uses the regional NWP chain on top of ECMWF IFS for the precipitation layer — that's a complete picture of global precipitation, but it's modelled output, not direct radar observation
